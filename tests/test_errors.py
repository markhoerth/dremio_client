...
def test_catalog(requests_mock):
    requests_mock.get('http://localhost:9047/api/v3/catalog',
...
def test_catalog_item(requests_mock):
    requests_mock.get('http://localhost:9047/api/v3/catalog/a',
                      status_code=401, reason="Unauthorized for url")
    with pytest.raises(DremioUnauthorizedException):
        catalog_item("1234", 'http://localhost:9047', 'a', 'b')
    requests_mock.get('http://localhost:9047/api/v3/catalog/b',
                      status_code=403, reason="Unauthorized for url")
    with pytest.raises(DremioPermissionException):
        catalog_item("1234", 'http://localhost:9047', 'b', 'b')
    requests_mock.get('http://localhost:9047/api/v3/catalog/by-path/b',
                      status_code=404, reason="Unauthorized for url")
    with pytest.raises(DremioNotFoundException):
        catalog_item("1234", 'http://localhost:9047', None, 'b')


def test_sql(requests_mock):
    requests_mock.post('http://localhost:9047/api/v3/sql',
                      status_code=401, reason="Unauthorized for url")
    with pytest.raises(DremioUnauthorizedException):
...
def test_jobstatus(requests_mock):
    requests_mock.get('http://localhost:9047/api/v3/job/1',
                      status_code=401, reason="Unauthorized for url")
    with pytest.raises(DremioUnauthorizedException):
